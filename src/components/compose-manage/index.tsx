...
export default function ComposeManage({
  ...
  const [pageSize, setPageSize] = useState(
    get(tableProps, 'pagination.pageSize', dataRequest?.params?.pageSize || 20),
  );
  // 当前页码
  ...
  const handleSearch = (values: Store = {}) => {
    filterProps?.map((item) => {
      ...
      if(item.type === 'dateRangePicker') {
        ...
        moment(values[item.name][0]).format(item.props?.format || 'YYYY-MM-DD'),
        moment(values[item.name][1]).format(item.props?.format || 'YYYY-MM-DD')] : []
      }
